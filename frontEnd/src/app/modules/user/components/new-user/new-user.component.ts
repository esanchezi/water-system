...
export class NewUserComponent implements OnInit {
  ...
  private initForm(): void {
    this.userForm = this.fb.group({
      ...
      alias:              [''],
      // Sin default a "true" a propósito -- que se marque explícitamente,
      // así no se activa el censo/uso solo por crear el usuario.
      habitaDomicilio:    [false, Validators.required],
      tieneToma:          [false, Validators.required],
      observaciones:      [''],
      email:              [''],
  ...
  private aplicarCopia(): void {
    ...
    this.userForm.patchValue({
      ...
      habitaDomicilio:    u.habitaDomicilio ?? false,
      tieneToma:          u.tieneToma ?? false,
      email:              u.email ?? '',
      nombre:             u.nombre ?? '',
      nombre2:            u.nombre2 ?? '',
      app:                u.app ?? '',
